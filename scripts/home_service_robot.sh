...
xterm -e "cd ${ws_path}; source devel/setup.bash; rosrun add_markers add_markers _param:=home_service_robot" &
sleep 1
echo "Running add_markers node is finished"

echo "Running pick_objects node..."
xterm -e "cd ${ws_path}; source devel/setup.bash; rosrun pick_objects pick_objects _param:=home_service_robot" &
